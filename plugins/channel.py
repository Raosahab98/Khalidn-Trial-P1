...
async def send_movie_updates(bot, file_name, caption):
    try:
        ...
        search_movies = movie_name.replace(" ", '-')
        caption_message = f"#New_File_Added ✅\n\nFile_Name:- <code>{movie_name}</code>\n\nLanguage:- {language}\n\nQuality:- {quality}"        
        btn = [
            [InlineKeyboardButton('Get File', url=f"https://telegram.me/{temp.U_NAME}?start=getfile-{search_movies}")]
        ]
        reply_markup = InlineKeyboardMarkup(btn)
        if poster_url:
            await bot.send_photo(MOVIE_UPDATE_CHANNEL, 
